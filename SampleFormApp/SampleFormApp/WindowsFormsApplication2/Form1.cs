...
namespace WindowsFormsApplication2
{
    public partial class Form1 : Form
    {
        ...
        private void btn_form_submit_Click(object sender, EventArgs e)
        {
            ...
            try
            { 
                String query = "INSERT INTO samples(sampleID, age, gender, duration, emotion) VALUES('"+txt_sampleID.Text+"', '"+txt_Age.Text+"', '" + ((KeyValuePair<string, string>)combo_gender.SelectedItem).Key+ "', 20, '" + ((KeyValuePair<string, string>)combo_emotions.SelectedItem).Key + "');";
                MySqlCommand cmd = new MySqlCommand(query,conn);
                conn.Open();
                cmd.ExecuteNonQuery();
                conn.Close();
